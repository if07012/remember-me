...
export default function Page({ params }: { params: { level: string } }) {
    ...
    useEffect(() => {
        const callApi = async () => {
            const response = await fetch(`https://bengkel-api-db-a0gpcsexa5cwe9g2.southeastasia-01.azurewebsites.net/api/sheet?sheet=Math-${params.level}`,
                {
                    method: 'GET',
                    headers: {
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100">
            <h1 className="text-6xl font-bold mb-6">Remember Math - {level}</h1>
                    ...
                    {!loading && questions.map(m => {
                        return <div className="bg-white p-6 rounded-lg shadow-md">
                            ...
                            <div className="grid grid-cols-3 gap-4">
                                ...
                                <label
                                    className={`block p-4 rounded-lg text-center cursor-pointer transition duration-300`}
                                    onClick={() => {
            ...
            <div className="grid grid-cols-2 gap-4">
                ...
                <button
                    ...
                    onClick={() => {
                        router.push(`/quiz/math/${level}/start`)
                    }}
                >
